...
-->
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Agregar Tipo de Maltrato</h5>
  ...
  <script>
    ...
    form.addEventListener('submit', function(event) {
      ...
      // Realizar una solicitud AJAX para validar el nombre del tipo de maltrato
      var xhr = new XMLHttpRequest();
      xhr.open('POST', 'validar_tipo_maltrato.php', true);
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState === XMLHttpRequest.DONE) {
          if (xhr.status === 200) {
            ...
            if (response === 'existe') {
              alert('El tipo de maltrato ya existe');
            } else if (response === 'ok') {
              // El tipo de maltrato es válido, puedes enviar el formulario
              form.submit();
            } else {
              alert('Error al validar el tipo de maltrato');
            }
          } else {
 	...
 	<div class="tabla" style="margin:0 auto;text-align:center;">
		...
		$consulta= mysqli_query($conection,"SELECT * FROM tipo_maltrato");
		...
		<tr>
		<td><?php echo $fetch["id_tipo_maltrato"]; ?></td>
		...
		<td>
		<button  class="btn btn-danger delete" data="<?php echo $fetch["id_tipo_maltrato"]; ?>"   title="Eliminar"><i class="fas fa-trash-alt"></i></button>
			<button type="button" class="btn btn-success mostrar_datos" id="mostrar_datos" data-toggle="modal" data-target="" href="editar_tipo_maltrato.php"
...
<style type="text/css">
  ...
  <div class="modal-body">
    <?php require_once "editar_tipo_maltrato.php";?>
...
$(document).on("click",".delete",function(e) {
  ...
  $.ajax({
    url:"eliminar_tipo_maltrato.php",
    type:"POST",
    data: dataString,
    beforeSend: function()
    {
    swal({
    text:'¡Tipo de maltrato eliminado!',
    type: 'success',
    cancelButtonText:'Cancelar',
    showCancelButton: true,
    confirmButtonColor: '#3085d6',
 ...
 <script type="text/javascript">
  ...
  var datos;
    ...
    id_tipo_maltrato=datos.find('td:eq(0)').text();
    nombre_maltrato=datos.find('td:eq(1)').text();
    
      $("#id_tipo_maltrato").val(id_tipo_maltrato);
      $("#nombre_maltrato").val(nombre_maltrato);
